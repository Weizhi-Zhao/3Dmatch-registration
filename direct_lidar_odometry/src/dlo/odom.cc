...
// MySerialPort STM32Serial("/dev/USART0");
...
dlo::OdomNode::OdomNode(ros::NodeHandle node_handle) : nh(node_handle) {
  ...
  // #ifdef USART_STM
  //   // 打开串口并设置参数
  // if (!STM32Serial.openport()) {
  //     perror("无法打开接收串口");
  // }
  // if (!STM32Serial.setoptions(115200, 8, 'N', 1)) {
  //     perror("发送串口参数设置失败");
  // }
  // STM32Serial.SetProcessData(&MY_ProcessData_From32);
  // #endif
  this->getParams();
...
void dlo::OdomNode::publishPose() {
  ...
  float coordinate[2];
  coordinate[0] = this->pose[0];
  coordinate[1] = this->pose[1];
  printf("x: %lf, y: %lf \n",coordinate[0], coordinate[1]);
  // Mid360RadarSend(coordinate, STM32Serial);

  std::ofstream file("/home/hustrobocon/desktop/livox_ws/xy_coordinates.txt", std::ios::app); // 使用 std::ios::app 表示在文件末尾追加
  if (file.is_open()) {
